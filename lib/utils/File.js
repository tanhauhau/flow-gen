...
const parserOptions = {
  sourceType: "module",
  plugins: ["jsx", "flow", "dynamicImport", "classProperties", "objectRestSpread"]
};
          ...
          switch (declaration.type) {
            case "FunctionDeclaration":
              if (isAnnotatedFunction(declaration)) {
                path.replaceWith(createFunctionDeclaration(declaration.id.name, declaration.params, declaration.returnType));
              }
...
function isAnnotatedFunction(node) {
  return !!node.returnType && node.params.every(param => !!param.typeAnnotation);
}

function createFunctionDeclaration(name, params, returnType) {
  return t.declareExportDeclaration(t.declareFunction({
    type: "Identifier",
    name,
    optional: false,
    typeAnnotation: t.typeAnnotation(t.functionTypeAnnotation(null, params.map(param => {
      return t.functionTypeParam(t.identifier(param.name), param.typeAnnotation.typeAnnotation);
    }), null, returnType.typeAnnotation))
  }));
}
